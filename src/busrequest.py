...
class BusRequest(db.Model):
    ...
    status_code = db.StringProperty(required=True, choices=set([BUSREQUEST_REQUESTED, BUSREQUEST_RETURNED_W_RESULTS, BUSREQUEST_RETURNED_WO_RESULTS]))
    last_modified = db.DateTimeProperty(auto_now=True)

    def isRequestReturned(self):
        return self.status_code == BUSREQUEST_RETURNED_W_RESULTS or self.status_code == BUSREQUEST_RETURNED_WO_RESULTS
